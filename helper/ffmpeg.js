...
module.exports=function(options, callback){
  if(!options.baseName) return callback(new TypeError("You most specify a baseName"))
  let folder=options.folder || os.tmpDir() //defaults
  let baseName=options.baseName
  let fileSrc = path.join(folder,`${baseName}-%d.jpg`)
  let fileDest =  path.join(folder,`${baseName}.webm`)

  //ffmpeg -i images-%d.jpeg -filter:v "setpts=2.5*PTS" -vcodec libvpx an video.webm
